...
{
    int s = 4, i, k, j, len, slen = 20, max_iters = 100;
    double h, alpha, beta, eta, xi, lambda_max, lambda_min, rGr, res0, res, tol = 1e-5;
    ...
    for (j = 0; j < 2*s; j++) {
        if (j == 0 || j == s+1) {
            B[index(j,j,2*s+1)] = xi / eta;
            B[index(j+1,j,2*s+1)] = 1./eta;
        }
        else if (j != s) {
            B[index(j,j,2*s+1)] = xi / eta;
            B[index(j-1,j,2*s+1)] = 1./(2 * eta);
            B[index(j+1,j,2*s+1)] = B[index(j-1,j,2*s+1)];
        }
    }
    ...
    for (k = 0; k < max_iters && res/res0 > tol; k++) {
        for (j = 0; j < 2*s+1; j++) {
            if (j == 0) {
                ...
                for (i = 0; i < len; i++)
                    V[index(j,i,len)] = eta * matrix_vec(A, &V[index(j-1,0,len)], len, len)[i] - xi * V[index(j-1,i,len)];
            }
            else {
                for (i = 0; i < len; i++)
                    V[index(j,i,len)] = 2 * (eta * matrix_vec(A, &V[index(j-1,0,len)], len, len)[i] - xi * V[index(j-1,i,len)]) - V[index(j-2,i,len)];
            }
        }
